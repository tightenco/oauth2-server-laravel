...
class FluentStorageServiceProvider extends ServiceProvider
{
    ...
    public function registerStorageBindings()
    {
        ...
        $this->app->bindShared('LucaDegasperi\OAuth2Server\Storage\FluentClient', function ($app) use ($provider) {
            $limitClientsToGrants = $app['config']->get('oauth2-server.limit_clients_to_grants');
        ...
        $this->app->bindShared('LucaDegasperi\OAuth2Server\Storage\FluentScope', function ($app) use ($provider) {
            $limitClientsToScopes = $app['config']->get('oauth2-server.limit_clients_to_scopes');
            $limitScopesToGrants = $app['config']->get('oauth2-server.limit_scopes_to_grants');
    ...
    {
        return ($this->app['config']->get('oauth2-server.database') !== 'default') ? $this->app['config']->get('oauth2-server.database') : null;
    }
}
